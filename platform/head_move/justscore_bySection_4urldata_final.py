...
def get_live_position(trueTime, start_time):
    ...
    rel_time = (trueTime - start_time)/2.5
...
def main(url, outputPth, designPth):
    ...
    try:
        ...
        df.loc[:, 'timestamp'] = df['timestamp'] - \
            startTime                          # set start time with 0 ms
        # detect index of rows with nan data
        idx = df.isna().sum(axis=1) == 0
        # discard rows with nan data
        df = df[idx].copy()
    ...
    #             plt.subplot(2,1,2)
                ...
                img = plt.imread(str(designPth)+"/2.png")
                ...
                outfileName = os.path.join(
                    outputPth, 'trail_'+str(level)+'.jpg')
                fig.savefig(outfileName, dpi=300)
                plt.close()
                # en version
                fig_en = plt.figure(figsize=(8.5, 6))
                # reading backgram photos with png format
                img_en = plt.imread(str(designPth)+"en/en_2.jpg")
                ...
                plt.tight_layout()
                outfileName = os.path.join(
                    outputPth, 'trail_en_'+str(level)+'.jpg')
            ...
            if state == 2 and level == 1:
                ...
                plt.subplots_adjust(left=0.01, bottom=0.01,
                                    right=0.99, top=0.97)
                # reading backgram photos with png format
                img = plt.imread(str(designPth)+"1.png")
                # location of canvas
                plt.imshow(img, extent=[-3.84, 3.84, -2.16, 2.16])
                ...
                plt.subplots_adjust(left=0.01, bottom=0.01,
                                    right=0.99, top=0.97)
                # reading backgram photos with png format
                img_en = plt.imread(str(designPth)+"en/en_1.png")
                # location of canvas
                plt.imshow(img_en, extent=[-3.84, 3.84, -2.16, 2.16])
                plt.plot(x, y, 'r-', linewidth=0.5)
                plt.xticks([])
        ...
        # # # - - - - - - - - visualization - - - - - - - - - -
                ...
                plt.subplots_adjust(left=0.01, bottom=0.01,
                                    right=0.99, top=0.97)
                # reading backgram photos with png format
                img = plt.imread(str(designPth)+str(level)+".png")
                # location of canvas
                plt.imshow(img, extent=[-3.84, 3.84, -2.16, 2.16])
                plt.plot(x, y, 'r-', linewidth=0.5)
                x0 = ROI[0][0]
    ...
    #             plt.title(f'第{level}题({thing})')
        ...
        logger.error(
            "Faild to open sklearn.txt from logger.error", exc_info=True)
    return
