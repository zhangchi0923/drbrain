...
class EyeScreen(object):
    base_cat = ['abs', 'calc4', 'calc5', 'calc6',
                'exec', 'mem8', 'mem9', 'mem10', 'recall']
    eye_cat = ['_aoi_ratio', '_ffd', '_ttff', '_nfbff']

    cog_cat = ['att'] + [x[0] + x[1]
                         for x in itertools.product(base_cat, ['_aoi_ratio'])]
    num_cols = ['att'] + [x[0] + x[1]
                          for x in itertools.product(base_cat, eye_cat)]
    ...
    def preprocess_feat(self, df) -> pd.DataFrame:
        ...
        # raise ValueError if #level-2 data is null
        x = model.predict(X)[:, 0]
        y = model.predict(X)[:, 1]
        ...
        result = pd.DataFrame(
            [self._gender, self._education, self._age] + feats).T
