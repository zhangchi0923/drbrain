...
class EyeMovement():
    ...
    def AOI_first_fixation(self, fix_data, combine_adjacent=False):
        '''
        Find first fixation located in AOI among merged fixations.
        :param fix_data:merged_fixations -- list of lists which contain start_time and end_time, durations and mean fixation center
        # :param combine_adjacent -- whether to combine adjacent fixations if time interval is under certain threshold
        return first AOI fixation duraion[1], time to first fixation[2] and number of fixations[2] before first fixation in AOI located in AOI.
        '''
        assert len(fix_data) > 0, 'Null fixation data.'
        ttff, nfbff = None, None
        for i, fix in enumerate(fix_data):
            if self.if_in_aoi(fix):
                if ttff is None or nfbff is None:
                    ttff = fix[0] - self._time_list[0]
                    nfbff = i
            else:
                if ttff is None or nfbff is None:
                    continue
        ...
        # case when no fixations in aoi
        if ttff is None and nfbff is None:
            return 0, 10000, 100
        
        if i <= nfbff+1:
            ffd = fix_data[nfbff][2]
        else:
            ffd = fix_data[i-1][1] - fix_data[nfbff][0]

        return ffd, ttff, nfbff
...
def test(s: str):
    ...
    eye_detector = EyeMovement(
    x = x,
    y = y,
    time_list = time,
    AOI = aoi
    )
    raw_results = eye_detector.eye_movements_detector(x, y, time, 100)
    prev_fix_num, merged_fix_num, merged_fixations = eye_detector.merge_fixation(raw_results, min_dur=75, max_time_interval=75)
    print(eye_detector.AOI_fixations(merged_fixations))
...
if __name__ == "__main__":
    ...
    with open(r'E:\zc\Data\eyetrack_data\2023-2-24-b3bbeb39-1bc4-47bb-ad44-6a44c4681386.txt') as f:
        data_str = f.read()
